...
pub fn cli() -> Command {
    Command::new("gen")
        ...
        .arg(Arg::new("args").help("The file(s) / directory(s) to generate").required(true).num_args(1..))
        .arg(
            Arg::new("count")
                .short('n')
                .long("count")
...
pub fn exec(gctx: &mut GlobalContext, args: &ArgMatches) -> CliResult {
    ...
    let template_args: Vec<String> = args.get_many::<String>("args").unwrap().map(|s| s.to_string()).collect();

    if count.is_some() && template_args.len() != 1 {
        return Err(CliError::from("--count requires exactly one template argument"));
    }

    let case_insensitive_paths: HashSet<String> = template_args.iter().map(|s| s.to_ascii_lowercase()).collect();

    let entries = fs::read_dir(&templates_dir).map_err(|_| CliError::from("failed to read templates directory"))?;
    ...
    if expanded_paths.is_empty() {
        gctx.shell().warn("No templates found for the given file(s) / directory(s)");
        return Ok(());
    }
    ...
    if let Some(n) = count {
        ...
        for i in 0..n {
            ...
            if fs::copy(path, &dest_path).is_err() {
                gctx.shell().error(format!("Failed to copy template file ({path}) to {dest_path}"));
            } else {
                gctx.shell().note(format!("Generated template file: {file_name}"));
            }
        }
    } else {
        for path in expanded_paths {
            ...
            let dest_path = format!("{}/{}", gctx.cwd().to_string_lossy(), file_name);
            if fs::copy(&path, &dest_path).is_err() {
                gctx.shell().error(format!("Failed to copy template file ({path}) destination {dest_path}"));
            } else {
                gctx.shell().note(format!("Generated template file: {file_name}"));
            }
        }
